/** WebGL animated gradient background (adapted from a 21st.dev
 * community component). Changes for this codebase: removed the
 * Next.js "use client" directive (Vite SPA), default export kept.
 * Renders as an absolutely-positioned background layer — the PARENT
 * must be position: relative. Self-contained: no external deps. */
...
export default function AnimatedGradient({
  ...
  useEffect(() => {
    ...
    if (!gl) return; // graceful: parent's CSS background shows instead
      ...
      {noise && noise.opacity > 0 && (
        <div
          style={{
            position: "absolute",
            inset: 0,
            backgroundImage: `url("data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAADAAAAAwBAMAAAClLOS0AAAAElMVEUAAAAAAAAAAAAAAAAAAAAAAADgKxmiAAAABnRSTlMCCgkGBAVJOAVJAAAASklEQVQ4y2NgGAWjYBSMglEwCgY/YGRgZBQUYmJiZGQEkYwMjIyMgoKCjIyMIJKBgRFIMjIyAklGRkYGRkFBYEcwMDIyMjAOUQAA1I4HwVwZAkYAAAAASUVORK5CYII=")`,
...
function hexToRgba(hex: string): [number, number, number, number] {
  ...
  if (hex.startsWith("rgba(")) {
    ...
    r = parseInt(parts[0]) / 255; g = parseInt(parts[1]) / 255; b = parseInt(parts[2]) / 255; a = parseFloat(parts[3]);
