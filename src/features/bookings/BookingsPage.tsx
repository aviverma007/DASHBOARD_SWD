    ...
    <div className="sw-inv" style={{ minHeight: "100vh" }}>
      ...
      <PageBanner title="Bookings" sub={<>{fN(ROWS.length)} active bookings · data as on {AS_ON}</>}>
          <div ref={projRef} style={{ position: "relative" }}>
            ...
            {projOpen && (
              <div style={{ position: "absolute", top: "100%", left: 0, zIndex: 40, marginTop: 4, background: "#fff", color: "var(--ink)", border: "1.5px solid #cfd6e2", borderRadius: 10, boxShadow: "0 12px 34px rgba(20,33,61,.18)", padding: "8px 10px", maxHeight: 300, overflowY: "auto", minWidth: 260 }}>
      ...
      <div style={{ padding: "16px 20px 40px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(185px, 1fr))", gap: 12, marginBottom: 6 }}>
          ...
          <KPI k="Advance" v={CRf(rows.reduce((s, b) => s + Math.max(b.recN - b.demN, 0), 0))} s={`paid ahead of demands · with tax ${CRf(rows.reduce((s, b) => s + Math.max(b.rec - b.demT, 0), 0))}`} />
        </div>
